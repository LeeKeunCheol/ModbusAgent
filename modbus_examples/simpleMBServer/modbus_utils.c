 ...
 * Local Data
 ******************************************************************************/
#define CDLOG(...) { gwlog.writeLog(GWLOG_DEBUG, ##__VA_ARGS__); }
#define CELOG(...) { gwlog.writeLog(GWLOG_ERROR, ##__VA_ARGS__); }
#define CILOG(...) { gwlog.writeLog(GWLOG_INFO,  ##__VA_ARGS__); }

// Modbus slave definitions
#define IAM 1   // RTU id
#define BASE 0  // Base address 
//#define DATA_SIZE 4 // 32 bits, 4 bytes, or 2 16 bit words of simulated io.
#define DATA_SIZE 100 // 32 bits, 4 bytes, or 2 16 bit words of simulated io.
...
int readRegisters(int tty, unsigned char rtu, unsigned char func) 
{
	...
	if ((uchCRCHi != 0) & (uchCRCLo != 0)) {
		...
		CILOG("Return Holding Registers");
		uchCRCHi = 0xff;
		uchCRCLo = 0xff;
		...
		for(i=0; i < byteCount; i++ ) {
			CILOG("%02d:%02x",address+i,data[address+i]);
			//                    tmp=write(tty,&i,1);
...
int writeMultipleRegisters(int tty, unsigned char rtu, unsigned char func) 
{
	...
	CDLOG("Byte Count %02d",byteCount);
	...
	CILOG("Write Multiple Registers");
	tmp=write(tty,&rtu,1);
	onTheFlyCalcCrc( rtu );
...
int writeSingleCoil(int tty, unsigned char rtu, unsigned char func) 
{
	...
	onTheFlyCalcCrc( rtu );
	onTheFlyCalcCrc( func );
	CILOG("entering writeSingleCoil");
	...
	if (rc == OK) {
		...
		write(tty,&addressHi,1);
		onTheFlyCalcCrc( addressHi );

		write(tty,&addressLo,1);
		onTheFlyCalcCrc( addressLo );
		...
		write(tty,&uchCRCLo,1);
		write(tty,&uchCRCHi,1);
	}
	return( rc );
}
...
void mbrtu_receiver_start(IN gw_app_t *config)
{
	...
	//strcpy(serialDev,"/dev/pts/13");
	uchCRCHi = 0xFF;
	uchCRCLo = 0xFF; 

	(void)memset( data,0x00, DATA_SIZE );

	tty=openSerialPort(config->mb_device,0); // 9600 bardrate
